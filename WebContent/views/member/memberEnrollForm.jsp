        ...
        <form id="enroll-form" action="<%= contextPath %>/insert.me" method="post"> <!--회원 정보가 노출되지 않도록 post 방식으로 요청 보내고자 함-->
            ...
            <script>
                function idCheck() {
                    ...
                    // AJAX로 controller에 요청하기 -> AJAX 처리 오류 내용 확인하려면 브라우저(?) console을 확인해야 함
                    $.ajax({
                        url : "idCheck.me", // 이 url 처리해 줄 controller/servlet 필요함
                        data : {checkId : $userId.val()},
                        success : function(result) { // result 경우의 수 = "NNNNN" 또는 "NNNNY" 2가지 -> 문자열 동등 비교로 따지기 (전반적으로 필기 확인 필요)
                        	if (result == "NNNNN") { // 중복된 id가 있는 경우 -> 사용자가 입력한 이 id는 사용 불가
                        		...
                        		if (confirm("사용 가능한 ID입니다. 사용하시겠습니까?")) { // 사용자가 사용하겠다고 '확인' 버튼 누르면 = true 반환(? 정확히 기억 안 나니까, JavaScript 수업 내용 확인해보기)
